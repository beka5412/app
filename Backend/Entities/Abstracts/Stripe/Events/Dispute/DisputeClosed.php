...
use Backend\Types\Stripe\Events\Payload\EStripeDisputeStatus;
...
class DisputeClosed
{
    ...
    public function response()
    {
        ...
        $order = Order::find($stripe_payment_intent->order_id);
        if (empty($order))
        ...
        // if ($order->status_details === EOrderStatusDetail::CHARGEDBACK->value)
        if ($stripe_payment_intent->status === EStripePaymentIntentStatus::PAYMENT_FAILED->value)
        ...
        if ($status === EStripeDisputeStatus::LOST->value)
        {
            ...
            SellerBalance::taxChargeback($order);
            ...
            $response_data = new ResponseData(['status' => 'success', 'message' => 'Payment chargebacked.']);
            $response_code = new ResponseStatus('200 OK');
        }

        else
        {
            $response_data = new ResponseData(['status' => 'error', 'message' => 'Nothing.']);
            $response_code = new ResponseStatus('200 OK');
        }
