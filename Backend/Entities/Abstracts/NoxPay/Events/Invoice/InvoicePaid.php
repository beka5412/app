...
class InvoicePaid
{
    ...
    {
        (new Log)->write(base_path('logs/nox_invoicepaid_data.log'), json_encode($request, JSON_PRETTY_PRINT));
        ...
        $order->seller_credited_at = date('Y-m-d H:i:s', strtotime("$now + days")); 
        $order->queue_seller_credit = 1;
        $order->save();
