...
namespace Backend\Controllers\User\Product\Link;
...
class IndexController
{
    ...
    public function __construct(App $application)
    {
        ...
        $this->indexFile = 'frontend/view/user/products/links/indexView.php';
        $this->user = user();
    }
    ...
    public function index(Request $request, $product_id)
    {
        ...
        $pixels = Pixel::where('user_id', $user->id)->orderBy('id', 'DESC')->paginate(10);
        $first_checkout = Checkout::where('product_id', $product->id)->where('user_id', $user->id)->orderBy('id', 'DESC')->first();
        $default_checkout = Checkout::where('product_id', $product->id)->where('user_id', $user->id)->where('default', 1)->orderBy('id', 'DESC')->first();
        $plans = Plan::where('user_id', $user->id)->where('product_id', $product->id)->orderBy('id', 'DESC')->get();
        View::render($this->indexFile, compact('title', 'context', 'user', 'pixels', 'product', 'first_checkout', 'default_checkout', 'plans'));
    }
    ...
    public function element(Request $request)
    {
        ...
        $default_checkout = Checkout::where('product_id', $product->id)->where('user_id', $user->id)->where('default', 1)->orderBy('id', 'DESC')->first();
        $plans = Plan::where('user_id', $user->id)->where('product_id', $product->id)->orderBy('id', 'DESC')->get();
        View::response($this->indexFile, compact('title', 'context', 'user', 'pixels', 'product', 'first_checkout', 'default_checkout', 'plans'));
    }
