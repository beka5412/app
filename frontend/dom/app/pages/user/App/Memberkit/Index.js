App.User.App.Memberkit.Index = class Index extends Page {
    context = 'dashboard';
    title = '';
    className = 'App.User.App.Memberkit.Index';

    view(loaded, link) {
        return super.find(`${link?.full ? 'full/' : ''}user/app/memberkit/${this?.constructor?.name || ''}${this?.queryString || ''}`, loaded);
    }

    destroyMemberkitIntegration(srcElement) {
        let options = {
        ...
        fetch(`/ajax/actions/user/app/memberkit/${id}/destroy`, options).then(response => response.json()).then(body => {
            ...
            if (body?.status == "success") {
                ...
                toastError(__('Memberkit integration not found.'));
        });
    }
};
