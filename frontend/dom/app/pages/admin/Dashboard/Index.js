App.Admin.Dashboard.Index = class Index extends Page {
    context = 'dashboard';
    title = 'Admin Dashboard';
    className = 'App.Admin.Dashboard.Index';
    myChart;
    SalesChart = {
        startDate: null,
        endDate: null
    ...
    view(loaded, link) {
        return super.find(`${link?.full?'full/':''}admin/dashboard/${this?.constructor?.name||''}${this?.queryString||''}`, loaded);
    }

    pageLoaded() {
        if (Object.keys(tagJSON('dashboardInfo')).length < 1) return
    ...
    chart() {
        ...
        this.createChart(chartData.monthly.labels, chartData.monthly.data, this.addAnnotations(chartData.yearly.data));
    }
